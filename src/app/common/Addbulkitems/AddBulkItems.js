...
// const AddItemsInBulkDialog = ({ open, onClose, itemList, formik }) => {
...
// export default AddItemsInBulkDialog;
"use client";
...
const AddItemsInBulkDialog = ({ open, onClose, itemList, formik }) => {
  ...
  // Sync local state with form data whenever dialog opens
  useEffect(() => {
    if (open) {
      // Set item selection status based on line items
      const updatedItems = itemList.map((item) => {
        const exists = formik.values.line_items.some(
          (lineItem) => lineItem.item_id === item.id
        );
        return { ...item, selected: exists };
      });
      setItems(updatedItems);

      // Set quantities from existing line items or default to 1
      const itemQuantities = {};
      formik.values.line_items.forEach((lineItem) => {
        if (lineItem.item_id) {
          itemQuantities[lineItem.item_id] = lineItem.quantity;
        }
      });
  ...
  }, [open, itemList, formik.values.line_items]);
  ...
  // Add selected items to formik
  const handleAddBulkItems = () => {
    if (selectedItems.length === 0) return;

    const existingItemIds = new Set(
      formik.values.line_items.map((item) => item.item_id)
    );
  ...
  return (
    ...
    >
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center" px={3} py={2}>
        ...
        <IconButton size="small" onClick={onClose} sx={{ color: "#ff0000" }}>
          <CloseIcon />
        </IconButton>
      </Box>
      <Divider />
      ...
      <Box sx={{ display: "flex", height: "450px" }}>
        ...
        <Box sx={{ width: "55%", borderRight: "1px solid #e0e0e0", p: 2 }}>
  <TextField
    fullWidth
  ...
  {/* Scrollable Area - Only this section will scroll */}
  <Box
    sx={{
      height: "400px", // <-- Fixed height for scroll area
      overflow: "auto",
      pr: 1, // Optional: Add padding to prevent content touching scrollbar
    }}
  >
    {filteredItems.map((item) => (
      <Paper
      ...
      >
        <Box sx={{ display: "flex", justifyContent: "space-between", width: "100%" }}>
          ...
          {item.selected && (
            ...
            >
              <CheckCircleIcon sx={{ color: "#22dd2e", fontSize: "22px" }} />
            </Box>
          )}
        </Box>
      </Paper>
        ...
        <Box sx={{ width: "45%", display: "flex", flexDirection: "column" }}>
          <Box px={2} display="flex" justifyContent="space-between" alignItems="center" my={1}>
            <Box display="flex" alignItems="center">
              <Typography variant="subtitle1" sx={{ fontSize: "16px" }}>
                Selected Items
              </Typography>
          ...
          <Box sx={{ flexGrow: 1, overflow: "auto" }}>
            {selectedItems.length > 0 ? (
                ...
                >
                  ...
                  <Box sx={{ display: "flex", alignItems: "center" }}>
                    <IconButton
                      size="small"
                      onClick={() => handleQuantityAdjust(item.id, false)}
                      sx={{ backgroundColor: "#f5f5f5", width: "28px", height: "28px", color: "#2196f3" }}
                    >
                      <RemoveIcon fontSize="small" />
                    </IconButton>
                    <TextField
                      ...
                      size="small"
                      inputProps={{
                        style: { textAlign: "center", padding: "2px", width: "30px" },
                      }}
                      sx={{ mx: 1 }}
                    />
                    <IconButton
              ...
              <Box sx={{ flexGrow: 1, display: "flex", alignItems: "center", justifyContent: "center", mt: 6 }}>
