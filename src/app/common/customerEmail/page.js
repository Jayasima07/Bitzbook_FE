...
const EmailComposer = () => {
  ...
  const customerType = searchParams.get("email_type");
  ...
  useEffect(() => {
    const fetchRecipientDetails = async () => {
      ...
      try {
        ...
        setDocumentType(
          customerType === "customer" ? "salesorder" : "purchaseorder"
        );
  ...
  const handleSend = async () => {
    try {
      ...
      console.log(fromEmail, selectedTo, subject, "required fields");
      ...
      formData.append(
        ...
        selectedTo
          .map(
            (id) =>
              contactPersonEmails.find((c) => c.contact_person_id === id)?.email
          )
          .join(", ")
      );
      ...
      formData.append("email_type", customerType);
      ...
      if (response.data?.status === true) {
        ...
        if (customerType === "Vendor") {
          window.location.href = `/purchase/vendor/${recipientId}`;
        } else {
            ...
            >
              ...
              >
                ...
                {recipientDetails?.contact_name ||
                  (customerType === "customer" ? "Customer" : "Vendor")}
              </Typography>
            </Grid>
            <Divider />
          </Grid>
          ...
          <Box sx={styles.emailContainer}>
                      ...
                      renderValue={(selected) => (
                        ...
                        >
                          {selected.map((value) => {
                            ...
                            const initialName = `${contact.first_name || ""} ${
                              contact.last_name || ""
                            }`.trim();
            ...
            <Box sx={styles.attachmentChip}>
              <FormControlLabel
                ...
                label={`Attach ${
                  customerType === "customer" ? "Sales Order" : "Purchase Order"
                } PDF`}
              />
            </Box>
