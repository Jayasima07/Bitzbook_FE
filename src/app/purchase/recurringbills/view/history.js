...
const History = ({ details }) => {
  ...
  // If details or comments are not provided, return an empty div
  ...
  return (
    <Timeline position="right" sx={{ p: 0, m: 0 }}>
      {details.comments.map((comment, index) => {
        ...
        const datePart = comment.date_formatted ? comment.date_formatted.split(' ')[0] : '';
        const timePart = comment.time || (comment.date_formatted ? comment.date_formatted.split(' ')[1] : '');
        
        // Determine the description (use the one from the comment or default to "Bill created.")
        const description = comment.description || "Bill created.";
        ...
        return (
          <TimelineItem key={comment.comment_id || index} sx={{ minHeight: '80px' }}>
            ...
            <TimelineSeparator>
            <TimelineConnector sx={{ backgroundColor: theme.palette.primary.main,width:"1px" }} />
              <TimelineDot 
                sx={{ 
            ...
            <TimelineContent sx={{ py: 1, px: 2 }}>
              ...
              >
                ...
                <Typography variant="body2" fontSize={"12px"} color="text.secondary" sx={{ mt: 0.5 }}>
                  by {createdBy}
                </Typography>
              </Paper>
            </TimelineContent>
