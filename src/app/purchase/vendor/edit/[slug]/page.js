...
import OtherDetailsVendor from "../../../../common/otherdetailsvendor/OtherDetailsVendor";
...
const VendorForm = () => {
  ...
  const getVendorData = async (uniqueId) => {
    ...
    try {
      const response = await apiService({
        method: "GET",
        url: `/api/v1/contact/${uniqueId}?organization_id=${organization_id}`,
        file: false,
      });

      if (response?.data?.data) {
        const vendorData = response.data.data;
        setContactData(vendorData);

        // Format the data before setting it in formik
        const formattedData = {
          ...formik.initialValues,
          ...vendorData,
          contact_persons: Array.isArray(vendorData.contact_persons)
            ? vendorData.contact_persons
            : [],
          bank_details: Array.isArray(vendorData.bank_details)
            ? vendorData.bank_details
            : [],
          billing_address: vendorData.billing_address || {},
          shipping_address: vendorData.shipping_address || {},
          // Ensure these fields are properly set
          contact_type: "Vendor",
          customer_sub_type: vendorData.customer_sub_type || "Business",
          payment_terms: vendorData.payment_terms || 0,
          currency_code: vendorData.currency_code || "INR- Indian Rupee",
          status: vendorData.status || "active",
          status_formatted: vendorData.status_formatted || "Active",
        };
        ...
        showMessage("Failed to fetch vendor data", "error");
      }
    } catch (error) {
      console.error("Failed to fetch vendor data:", error);
      showMessage(
        error?.response?.data?.message ||
          "Failed to fetch vendor data. Please try again.",
        "error"
      );
    }
  };

  // Get vendor data in edit mode
  useEffect(() => {
    if (uniqueId && organization_id) {
      getVendorData(uniqueId);
    }
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        ...
        // Format the current date for updated_time
        ...
        // Make API call
        const response = await apiService({
          method: "PUT", // Use PUT for updates, POST for creation
        ...
        if (data?.contact_id) {
          ...
          router.push(`/purchase/vendor/${data.contact_id}`);
        } else {
          console.error("Invalid contact_id:", data);
          showMessage("Failed to retrieve vendor ID", "error");
        }
      } catch (error) {
  ...
  // Dynamically update display name options
  useEffect(() => {
    updateDisplayNameOptions();
  }, [
  ...
  const handleCancel = () => {
    router.push(`/purchase/vendor/${uniqueId}`);
  };
  ...
  useEffect(() => {
      if (
        ...
        updatedContactPersons[0] = {
          ...
          is_added_in_portal: formik.values.is_portal_enabled ? true : false,
          portal_access: false,
        };
        ...
        <Container maxWidth="md" sx={{ mt: 2, px: 2, ml: 1 }}>
                ...
                >
                  ...
                  >
                    <FormControlLabel
                      ...
                      label={<Typography sx={{ fontSize: "14px" }}>Business</Typography>}
                    />
                    <FormControlLabel
                      value="Individual"
                      control={<Radio sx={{ '& .MuiSvgIcon-root': { fontSize: 20 } }} />}
                      label={<Typography sx={{ fontSize: "14px" }}>Individual</Typography>}
                    />
                  </RadioGroup>
                </Grid>
              </Grid>
          ...
          <Box sx={{ width: "100%", mt: 5 }}>
              ...
              >
                <Tab
                  ...
                  sx={{
                    ...
                    borderBottom: tabValue === 0 ? "3px solid" : "none",
                  }}
                />
                <Tab
                  label="Address"
            ...
            <TabPanel value={tabValue} index={0}>
              <Grid sx={{ margin: -4 }}>
                <OtherDetailsVendor formik={formik} />
              </Grid>
            </TabPanel>
            ...
            {/* <TabPanel value={tabValue} index={3}>
              ...
              >
                Start adding custom fields for your Vendors and vendors by going
                to Settings Preferences Vendors and vendors. You can also refine
                the address format of your Vendors and vendors from there.
              </Typography>
            </TabPanel> */}
            ...
            <TabPanel value={tabValue} index={4}>
              <Typography
                sx={{ fontSize: "13px", textAlign: "center", margin: "30px" }}
              >
                You`ve not created any Reporting Tags. Start creating reporting
