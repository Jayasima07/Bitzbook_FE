...
import OtherDetailsCustomer from "../../../../common/otherdetailscustomer/OtherDetailsCustomer";
...
const CustomerForm = () => {
  ...
  const validationSchema = Yup.object({
    ...
    legal_name: Yup.string().when("gst_treatment", (gst_treatment, schema) => {
      if (formik.values.gst_treatment !== "") {
        return fieldVisibility[gst_treatment]?.includes("business_legal_name")
          ? schema.required("Business legal name is required")
          : schema.notRequired();
      }
      return schema.notRequired();
    }),

    trader_name: Yup.string().when("gst_treatment", (gst_treatment, schema) => {
      if (formik.values.gst_treatment !== "") {
        return fieldVisibility[gst_treatment]?.includes("business_trade_name")
          ? schema.required("Business trade name is required")
          : schema.notRequired();
      }
      return schema.notRequired();
    }),
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (data?.contact_id) {
          ...
          showMessage("Failed to retrieve customer ID", "error");
        }
      } catch (error) {
  ...
  const getCustomerData = async (uniqueId) => {
    ...
    try {
      ...
      // Populate Formik values with fetched data
      formik.setValues({
  ...
  const handleCancel = () => {
    router.push(`/sales/customer/${uniqueId}`);
  };
        ...
        <Container maxWidth="md" sx={{ mt: 2, px: 2, ml: 1 }}>
          ...
          <Box sx={{ width: "100%", mt: 5 }}>
            ...
            <TabPanel value={tabValue} index={3}>
              ...
              >
                Start adding custom fields for your Customers and customers by
                going to Settings Preferences Customers and customers. You can
                also refine the address format of your Customers and customers
                from there.
              </Typography>
            </TabPanel>
